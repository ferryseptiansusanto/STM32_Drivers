...
//  I2C_ScanBus(&hi2c1);   // kalau pakai I2C1
//  xTaskCreate(cmd_collect_task, "cmd_collect_task", 256, NULL, 2, NULL);
//  xTaskCreate(vTaskCommand, "command_task", 256, NULL, 2, NULL);

  //Buat Mutex supaya SPI tidak bentrok
//  SD_Card_Mutex = xSemaphoreCreateMutex();
  ...
  //Init LCD
  LCD_Init(&Lcd_Ctx, &i2c1_ctx);
  //Init Keypad
  KEYPAD_Init(&Keypad_Ctx, &i2c2_ctx); // jika 4x4
  //Buat task logger
//  LCD_TaskCreate(tskIDLE_PRIORITY+1);
  //Buat task lcd
  LOGGER_TaskCreate(&SDCard_Ctx, &DS3231_Ctx, tskIDLE_PRIORITY+1);
  // buat task aplikasi (menu + LCD)
  APP_TaskCreate(tskIDLE_PRIORITY+3);
  // buat task keypad
  KEYPAD_TaskCreate(&Keypad_Ctx, tskIDLE_PRIORITY+2);
  COMMAND_TaskCreate(tskIDLE_PRIORITY+2);
  // Start scheduler
  vTaskStartScheduler();
